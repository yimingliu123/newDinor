...
#     "dirt track", "apparel", "pole", "land", "bannister", "escalator", "ottoman", "bottle", "buffet", 
#     "poster", "stage", "van", "ship", "fountain", "conveyer belt", "canopy", "washer", "plaything", 
#     "swimming pool", "stool", "barrel", "basket", "waterfall", "tent", "bag", "minibike", "cradle", 
#     "oven", "ball", "food", "step", "tank", "trade name", "microwave", "pot", "animal", "bicycle", 
#     "lake", "dishwasher", "screen", "blanket", "sculpture", "hood", "sconce", "vase", "traffic light", 
#     "tray", "ashcan", "fan", "pier", "crane", "hovel", "counter", "oven", "refrigerator", "desk", 
#     "wardrobe", "staircase"
# ]
...
# 数据集路径
DATASET_ROOT = ''
ANN_ROOT = os.path.join(DATASET_ROOT, 'annotations_detectron2')

# training/validation;train2017/val2017
TRAIN_PATH = os.path.join(DATASET_ROOT, 'images/train2017')
VAL_PATH = os.path.join(DATASET_ROOT, 'images/val2017')

TRAIN_PNG = os.path.join(ANN_ROOT, 'train2017')
#VAL_JSON = os.path.join(ANN_ROOT, 'val.json')
VAL_PNG = os.path.join(ANN_ROOT, 'val2017')

# 声明数据集的子集
PREDEFINED_SPLITS_DATASET = {
    "coco_my_train": (TRAIN_PATH, TRAIN_PNG),
    "coco_my_val": (VAL_PATH, VAL_PNG),
}
...
def register_dataset():
    ...
    """
    for key, (image_root, png_root) in PREDEFINED_SPLITS_DATASET.items():
...
#注册数据集实例，加载数据集中的对象实例
def register_dataset_instances(name, gt_root, image_root):
    """
    purpose: register dataset to DatasetCatalog,
             register metadata to MetadataCatalog and set attribute
    """
    DatasetCatalog.register(name, lambda: load_sem_seg(gt_root, image_root))
    MetadataCatalog.get(name).set(gt_root=gt_root,
                                  image_root=image_root,
                                  ignore_label=255,  # 缺少这个标签，加上
                                  stuff_classes = NATION_RICE_CLASS_NAME,
...
def plain_register_dataset():
    #训练集
    DatasetCatalog.register("coco_2017_train", lambda: load_sem_seg(TRAIN_PNG, TRAIN_PATH))
    MetadataCatalog.get("coco_2017_train").set(thing_classes=NATION_RICE_CLASS_NAME,  # 可以选择开启，但是不能显示中文，这里需要注意，中文的话最好关闭
                                                    evaluator_type='sem_seg', # 指定评估方式
                                                    json_file=TRAIN_PNG,
                                                    ignore_label=255,  # 缺少这个标签，加上
    ...
    #验证/测试集
    DatasetCatalog.register("coco_2017_val", lambda: load_sem_seg(VAL_PNG, VAL_PATH))
...
def checkout_dataset_annotation(name="coco_my_val"):
    ...
    dataset_dicts = load_sem_seg(TRAIN_PNG, TRAIN_PATH)
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    cfg.OUTPUT_DIR = args.OUTPUT_DIR # 自定义输出路径
    cfg.TEST.AUG.ENABLED = args.use_tta if args.eval_only else False # 自定义增强测试
...
if __name__ == "__main__":
    ...
    parser.add_argument('--OUTPUT_DIR', type=str, default="output/sem/20241205_Version2/nation_rice_sem_output_swin_base_IN21k") # 自定义模型输出路径
    parser.add_argument('--use_tta', action='store_true') # 自定义模型输出路径
